...
from core.candidates import generate_candidate_pairs_2hop, generate_candidate_pairs_knn
from core.model import load_catboost_model, load_feature_cols, predict_pairs_catboost
...
def run_inference_pipeline(
    ...
    if len(profile_df) < 2:
        return (
            ...
            {'profiles': len(profile_df), 'pairs': 0, 'model_mode': 'none'},
        )

    candidate_pairs = generate_candidate_pairs_2hop(profile_df, ATTRIBUTE_COLS)
    candidate_pairs |= generate_candidate_pairs_knn(profile_df, NUMERIC_COLS, n_neighbors=10)

    if not candidate_pairs:
        return (
            pd.DataFrame(columns=['profile_id_1', 'profile_id_2', 'score']),
            {'profiles': len(profile_df), 'pairs': 0, 'model_mode': 'no_pairs'},
        )
    ...
    if cat_model is None or feature_cols is None:
        raise RuntimeError(
            "Не найдена модель или feature_cols.json. "
            "Положи артефакты в папку artifacts/."
        )
